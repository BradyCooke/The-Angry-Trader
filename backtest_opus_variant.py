#!/usr/bin/env python3
"""
Opus 4.5 Variant Backtest

My suggestions focus on reducing false breakouts and improving trade quality:
1. Momentum confirmation - require 2 consecutive closes beyond band
2. Volume confirmation - require above-average volume on breakout
3. Cooldown period - no re-entry on same symbol for 10 days after stop-out
4. Wider initial stops (3.0× ATR) to reduce whipsaws
5. Delayed trailing activation (2.0× ATR profit) to let trades develop
"""
...
class Position:
    ...
    def update_trailing_stop(
        ...
        """Update trailing stop with delayed activation."""
        if self.is_long:
            self.highest_high = max(self.highest_high, current_high)
            profit_threshold = self.entry_price + (trailing_activation_atr * self.atr_at_entry)
...
class Portfolio:
    def __init__(self, starting_capital: float = 100000.0):
        ...
        self.cooldowns: dict[str, date] = {}  # Opus: track cooldown periods
    ...
    def is_on_cooldown(self, symbol: str, current_date: date, cooldown_days: int) -> bool:
        """Check if symbol is on cooldown after a stop-out."""
        if symbol not in self.cooldowns:
            return False
        cooldown_end = self.cooldowns[symbol] + timedelta(days=cooldown_days)
    ...
    def close_position(self, symbol, exit_price, exit_date, exit_reason="signal"):
        ...
        # Opus: Set cooldown if stopped out
        if exit_reason == "stop":
...
class OpusSignalGenerator:
    def __init__(
        self,
        ema_period: int = 35,
        atr_period: int = 20,
        atr_multiplier: float = 2.0,
        initial_stop_atr: float = 3.0,  # Opus: wider stops to reduce whipsaws
        trailing_stop_atr: float = 2.5,
        trailing_activation_atr: float = 2.0,  # Opus: delayed activation
        volume_lookback: int = 20,
        volume_threshold: float = 1.2,  # Opus: require 20% above avg volume
        momentum_bars: int = 2,  # Opus: require 2 consecutive closes
    ):
    ...
    def generate_signals(self, df: pd.DataFrame, symbol: str) -> list[Signal]:
        min_period = max(self.ema_period, self.atr_period, self.volume_lookback, self.momentum_bars + 1)
        ...
        for i in range(self.momentum_bars, len(df)):
            row = df.iloc[i]
            if pd.isna(row["keltner_upper"]) or pd.isna(row["atr"]) or pd.isna(row["volume_ma"]):
                continue
            ...
            # Opus: Momentum confirmation - check last N bars
            momentum_long = True
            momentum_short = True
            for j in range(self.momentum_bars):
                prev_row = df.iloc[i - j]
                if prev_row["close"] <= prev_row["keltner_upper"]:
                    momentum_long = False
                if prev_row["close"] >= prev_row["keltner_lower"]:
                    momentum_short = False

            # Opus: Volume confirmation
            volume_confirmed = volume_ratio >= self.volume_threshold

            # Check for first bar crossing (need at least momentum_bars+1 bars ago inside)
            first_cross_bar = df.iloc[i - self.momentum_bars + 1]
            prev_first = df.iloc[i - self.momentum_bars]

            # LONG signal
            if (momentum_long and volume_confirmed and inside_band and
                first_cross_bar["close"] > first_cross_bar["keltner_upper"] and
                prev_first["close"] <= prev_first["keltner_upper"]):
            ...
            # SHORT signal
            elif (momentum_short and volume_confirmed and inside_band and
                  first_cross_bar["close"] < first_cross_bar["keltner_lower"] and
                  prev_first["close"] >= prev_first["keltner_lower"]):
...
class OpusBacktestEngine:
    def __init__(
        self,
        signal_gen: OpusSignalGenerator,
        volatility_target_pct: float = 2.0,
        max_position_pct: float = 15.0,
        max_gross_exposure_pct: float = 100.0,
        cooldown_days: int = 10,  # Opus: cooldown after stop-out
    ):
    ...
    def run(self, data, start_date=None, end_date=None):
        ...
        for current_date in dates:
            ...
            for sig in signals:
                ...
                # Opus: Check cooldown
                if portfolio.is_on_cooldown(sig.symbol, current_date, self.cooldown_days):
                    continue
...
def main():
    ...
    sig_gen = OpusSignalGenerator(
        ema_period=35,
        atr_period=20,
        atr_multiplier=2.0,
        initial_stop_atr=3.0,  # Opus: wider stops
        trailing_stop_atr=2.5,
        trailing_activation_atr=2.0,  # Opus: delayed activation
        volume_lookback=20,
        volume_threshold=1.2,  # Opus: volume confirmation
        momentum_bars=2,  # Opus: momentum confirmation
    )

    engine = OpusBacktestEngine(
        sig_gen,
        volatility_target_pct=2.0,
        max_position_pct=15.0,
        max_gross_exposure_pct=100.0,
        cooldown_days=10,  # Opus: cooldown after stop-out
    )
    ...
    print("=" * 80)
    print("OPUS 4.5 VARIANT RESULTS")
    print("=" * 80)
    print("\nKey Changes from Baseline:")
    print("  1. Momentum confirmation - require 2 consecutive closes beyond band")
    print("  2. Volume confirmation - require 20% above average volume")
    print("  3. Cooldown period - no re-entry for 10 days after stop-out")
    print("  4. Wider initial stops (3.0× ATR) to reduce whipsaws")
    print("  5. Delayed trailing activation (2.0× ATR profit)")
    print()
