#!/usr/bin/env python3
"""
Claude Variant Backtest

Implements Claude's suggestions:
1. Widen trailing stop to 3.5× ATR with 1.5× ATR activation delay
2. 200-day MA trend filter (only long above, only short below)
3. Increase Keltner multiplier to 2.5×
4. Breakout strength filter (require 0.5× ATR beyond band)
5. Time-based exit for non-performers (40 days)
6. Chandelier Exit (trailing from high/low extremes)
"""
...
class ClaudeSignalGenerator:
    def __init__(
        ...
        ema_period: int = 35,
        atr_period: int = 20,
        atr_multiplier: float = 2.5,  # Claude: wider channels
        initial_stop_atr: float = 2.5,
        trailing_stop_atr: float = 3.5,  # Claude: wider trailing
        trailing_activation_atr: float = 1.5,  # Claude: delayed activation
        trend_filter_period: int = 200,  # Claude: 200-day MA filter
        breakout_strength_min: float = 0.5,  # Claude: require 0.5× ATR beyond band
        time_stop_days: int = 40,  # Claude: time-based exit
    ):
        ...
        self.breakout_strength_min = breakout_strength_min
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["ma_200"] = df["close"].rolling(window=self.trend_filter_period).mean()
        return df

    def generate_signals(self, df: pd.DataFrame, symbol: str) -> list[Signal]:
        min_period = max(self.ema_period, self.atr_period, self.trend_filter_period)
        ...
        for i in range(1, len(df)):
            ...
            if pd.isna(row["keltner_upper"]) or pd.isna(row["atr"]) or pd.isna(row["ma_200"]):
                continue
            ...
            # LONG: close above upper band, was inside, and above 200 MA (trend filter)
            if close > upper and prev_close <= prev_upper and inside_band:
                strength = (close - upper) / atr if atr > 0 else 0
                # Claude: require minimum breakout strength
                if strength >= self.breakout_strength_min and close > ma_200:
                    ...
                    signals.append(Signal(
                        ...
                        metadata={"ma_200": ma_200}
                    ))
                inside_band = False

            # SHORT: close below lower band, was inside, and below 200 MA (trend filter)
    ...
    def run(self, data, start_date=None, end_date=None):
        ...
        for current_date in dates:
            ...
            for sym, pos in list(portfolio.positions.items()):
                ...
                # Time stop (Claude suggestion)
                if pos.days_held(current_date) >= self.signal_gen.time_stop_days:
                    # Only exit if not profitable
                    pnl = pos.calculate_pnl(close)
                    if pnl <= 0:
                        symbols_to_close.append((sym, close, "time_stop"))
                        continue

                # Update trailing stop (Chandelier Exit style)
...
def main():
    ...
    sig_gen = ClaudeSignalGenerator(
        ema_period=35,
        atr_period=20,
        atr_multiplier=2.5,  # Claude: wider channels
        initial_stop_atr=2.5,
        trailing_stop_atr=3.5,  # Claude: wider trailing
        trailing_activation_atr=1.5,  # Claude: delayed activation
        trend_filter_period=200,  # Claude: 200-day MA filter
        breakout_strength_min=0.5,  # Claude: require 0.5× ATR beyond band
        time_stop_days=40,  # Claude: time-based exit
    )

    engine = ClaudeBacktestEngine(
        sig_gen,
    ...
    print("=" * 80)
    print("CLAUDE VARIANT RESULTS")
    print("=" * 80)
    print("\nKey Changes from Baseline:")
    print("  1. Wider trailing stop (3.5× ATR) with delayed activation (1.5× ATR)")
    print("  2. 200-day MA trend filter (long above, short below)")
    print("  3. Wider Keltner channels (2.5× ATR multiplier)")
    print("  4. Breakout strength filter (require 0.5× ATR beyond band)")
    print("  5. Time-based exit for non-performers (40 days)")
    print("  6. Chandelier Exit style trailing stop")
    print()
